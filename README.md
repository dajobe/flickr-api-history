...
`api-changes.md` is a hand-written summary of the API changes observed in
this archive.  It is not generated automatically from `methods.lst` or the HTML
archives.
...
For a manually curated, newest-first summary, see `api-changes.md`.
...
After committing a method-list or archived documentation change, update
`api-changes.md` by hand.  The change history is intentionally not generated by the
Makefile, because the raw diffs often need interpretation.
